package top.mortise.utils.QRCodeUtil;
...
public class QRCodeUtils {
    ...
    private static BufferedImage toBufferedColorImage(BitMatrix matrix) {
        ...
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                if( matrix.get(x, y) ){
                    int num = random.nextInt(colorPixels.length-1);
                    image.setRGB(x, y, colorPixels[num]);
                }else{
    ...
    private static BufferedImage toBufferedScaleImage(BitMatrix matrix, int width, int height,String scaleImagePath) {
        ...
        if(scaleImagePath!=null&&!scaleImagePath.equals("")) {
            try {
