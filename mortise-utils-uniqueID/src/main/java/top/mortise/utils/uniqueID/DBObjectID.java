...
import static top.mortise.utils.comm.Assertions.isTrueArgument;
import static top.mortise.utils.comm.Assertions.notNull;

/**
 * Created by Eric.Zhang on 2016/12/24.
 */
public class DBObjectID implements Comparable<DBObjectID>, Serializable {
     ...
     * @since 2.12.0
     */
    public static DBObjectID createFromLegacyFormat(final int time, final int machine, final int inc) {
        return new DBObjectID(time, machine, inc);
    }
     ...
     * @param date the date
     */
    public DBObjectID(final Date date) {
     ...
     */
    public DBObjectID(final Date date, final int counter) {
     ...
     */
    public DBObjectID(final Date date, final int machineIdentifier, final short processIdentifier, final int counter) {
     ...
     * @throws IllegalArgumentException if the high order byte of machineIdentifier or counter is not zero
     */
    public DBObjectID(final int timestamp, final int machineIdentifier, final short processIdentifier, final int counter) {
        this(timestamp, machineIdentifier, processIdentifier, counter, true);
    }

    private DBObjectID(final int timestamp, final int machineIdentifier, final short processIdentifier, final int counter,
     ...
     */
    public DBObjectID(final String hexString) {
     ...
     */
    public DBObjectID(final byte[] bytes) {
     ...
     */
    DBObjectID(final int timestamp, final int machineAndProcessIdentifier, final int counter) {
     ...
     * @since 3.4
     */
    public DBObjectID(final ByteBuffer buffer) {
    ...
    public boolean equals(final Object o) {
        ...
        DBObjectID objectId = (DBObjectID) o;
    ...
    @Override
    public int compareTo(final DBObjectID other) {
        if (other == null) {
    ...
    private static int createMachineIdentifier() {
        ...
        try {
            ...
            LogHelper.logger().warn( "Failed to get machine identifier from network interface, using random number instead", t);
        }
    ...
    private static short createProcessIdentifier() {
        ...
        try {
            ...
            LogHelper.logger().warn( "Failed to get process identifier from JMX, using random number instead", t);
        }
